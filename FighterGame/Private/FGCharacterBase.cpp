...
AFGCharacterBase::AFGCharacterBase()
{
	...
	CharacterCommands[1].inputs.Add("Q");
...
void AFGCharacterBase::BeginHitStop()
{
	...
	{v
		gameMode->istimeractive
	}
	*/
	GetWorld()->GetTimerManager().SetTimer(hitStopTimerHandle, this, &AFGCharacterBase::EndHitStop, hitStopTime, false);
}

void AFGCharacterBase::EndHitStop()
{
	CustomTimeDilation = 1.f;
	OtherPlayer->CustomTimeDilation = 1.f;
...
void AFGCharacterBase::PlayDamageEffects(FVector _hitLocation)
{
	UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), DamageParticle, _hitLocation);
...
void AFGCharacterBase::TakeDamage(float DamageAmount, float stunTime, float blockStunTime, float pushBackAmount, float launchAmount, FVector _hitLocation)
{
	if (CharacterState != ECharacterState::VE_Blocking)
	{
		...
		if (OtherPlayer)
		{
			...
			{
				OtherPlayer->SuperMeterAmount += DamageAmount * 1.f;
			}
		}
